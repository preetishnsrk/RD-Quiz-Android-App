...
public class MarathonLeaderboardAdapter extends RecyclerView.Adapter<MarathonLeaderboardAdapter.ViewHolder> {
    ...
    private List<MarathonLeaderboardModel> values;
    private Activity activity;
    private Resources resources;

    // Provide a suitable constructor (depends on the kind of dataset)
    public MarathonLeaderboardAdapter(Activity activityIn, List<MarathonLeaderboardModel> valuesIn, Resources resourcesIn) {
        activity = activityIn;
        values = valuesIn;
    ...
    @Override
    public MarathonLeaderboardAdapter.ViewHolder onCreateViewHolder(ViewGroup parent,
                                                            int viewType) {

        Context context = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(context);

        View view = inflater.inflate(R.layout.marathon_leaderboard_layout, parent, false);
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        MarathonLeaderboardModel leaderBoardModel = values.get(position);
        ...
        int correctPoints = Integer.parseInt(leaderBoardModel.getRight());
        int wrongPenalty = Integer.parseInt(leaderBoardModel.getWrong());
        int passPenalty = Integer.parseInt(leaderBoardModel.getPasses());
        holder.rightWrongAndPassTV.setText("Correct Answers:    "+leaderBoardModel.getRight()+" - earned "+(correctPoints*20)+"pts"
                +"\nIncorrect Answers:  "+leaderBoardModel.getWrong()+" - cost "+(wrongPenalty*20)+"pts"
                +"\nQuestions Passed:   "+leaderBoardModel.getPasses()+" - cost "+(passPenalty*10)+"pts");

        int time = Integer.parseInt(leaderBoardModel.getTimeTaken());
        int minutes = time/60;
        int seconds = time-(minutes*60);
        String timeTaken = minutes+"m"+seconds+"s";
        holder.totalScoreAndTimeTaken.setText(leaderBoardModel.getScore()+"pts in "+timeTaken);
